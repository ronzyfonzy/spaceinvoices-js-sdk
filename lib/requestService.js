    ...
    value: function call(endpoint) {
      ...
      return this.requester(requestProps).then(function (response) {
        ...
        if (response.statusCode >= 400 && response.statusCode < 500) {
          return Promise.reject(new _errors.RequestError(response.response.body.error));
        } else {
          return Promise.reject(new _errors.ResponseError(response.response.body.error));
        }
      });
    }
  }]);
